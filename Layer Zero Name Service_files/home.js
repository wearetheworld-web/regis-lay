...
const lzDomainAddress = '0x64ef4935521b90cef6622b778ebe10f4ed111dbe'
...
function wrongNetworkError() {
  Swal.fire('Error', "You need to switch your wallet's network to Ethereum Mainnet.", 'error')
}
...
function displayAccount(account) {
  ...
  if (account === undefined) {
    ...
    document.getElementById('desktopCW').innerHTML =
      account.substring(0, 6) + '......' + account.substring(account.length - 6, account.length)
    document.getElementById('mobilCW').innerHTML =
      account.substring(0, 6) + '......' + account.substring(account.length - 6, account.length)
  }
}
...
async function initialize() {
  lzContractRead = new readerWeb3.eth.Contract(lzDomainAbi, lzDomainAddress)
  lzContractWrite = new writerWeb3.eth.Contract(lzDomainAbi, lzDomainAddress)
  loadPage()
  if (typeof window.ethereum !== 'undefined') {
...
async function registerDomain() {
  ...
  try {
    ...
    var goRegister = await lzContractWrite.methods
      .register(name, year)
      .send({
        from: accounts[0].toUpperCase(),
        value: value,
      ...
      .once('transactionHash', function (hash) {
        ...
        Swal.fire({
          ...
          footer:
            ...
            ethereumParams.blockExplorerUrls[0] +
            '/tx/' +
            hash +
            '">Transaction Hash: ' +
            hash.substring(0, 10) +
